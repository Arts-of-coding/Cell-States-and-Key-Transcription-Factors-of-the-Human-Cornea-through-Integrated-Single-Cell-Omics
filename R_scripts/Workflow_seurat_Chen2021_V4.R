...
workdir <- "/ceph/rimlsfnwi/data/moldevbio/zhou/jarts/R/scRNA-seq_Li2021/"
...
path2 <- "/ceph/rimlsfnwi/data/moldevbio/zhou/jarts/data/Chen2021/"
...
for (i in list.files (path = path2, pattern = "GSM")){
  ...
  nam2 <- paste("B", i, sep = "")
  nam2 <- CreateSeuratObject(counts = nam, project = paste0("Li",i), min.cells = 3, min.features = 200)
...
seur_obj_all <- merge(Combined, y = GSM4646296, add.cell.ids = v2, project = "Li2021all")
...
seur_obj <- QCSeurObjClustering(SeuratObject = seur_obj,dimensions=16)

ClusteringResSeurObj(SeuratObject = seur_obj,dimensions=16)

chosen_res <- 0.1

# Final chosen resolution
pdf(paste(resultsdir,'8b.clusdendrogram_0.1_16dims.pdf',sep="/") ,width=8,height=8,paper='special')
seur_obj <- FindNeighbors(seur_obj, dims = 1:16)
res <- chosen_res
cluster_variable_name <- paste0("RNA_snn_res.", res)
seur_obj <- FindClusters(seur_obj, verbose = FALSE, resolution = res, graph.name = "RNA_snn")
seur_obj <- BuildClusterTree(seur_obj)
...
new.cluster.ids <- c('cell1','cell2','cell3','cell4','cell5','cell6','cell7','cell8','cell9','cell10')
seur_obj$costum_clustering <- plyr::mapvalues(x = seur_obj$RNA_snn_res.0.1, from = current.cluster.ids, to = new.cluster.ids)
cluster_order <- c('cell1','cell2','cell3','cell4','cell5','cell6','cell7','cell8','cell9','cell10')
...
saveRDS(seur_obj, file = (paste(resultsdir,'/Li_unannotated.rds',sep="/")))

marker_gene_file <- "/ceph/rimlsfnwi/data/moldevbio/zhou/jarts/R/LSC_Marker_Genes_Chen.csv"
...
cluster_order <- c('Li_C1_DC','Li_C2_Cj',
                   'Li_C3_DC','Li_C4_PC','Li_C5_DC',
                   'Li_C6_PC','Li_C7_Mel','Li_C8_TALSC',
                   'Li_C9_LSC','Li_C10_UN')
...
new.cluster.ids <- c('Li_C1_DC','Li_C2_Cj',
                     'Li_C3_DC','Li_C4_PC','Li_C5_DC',
                     'Li_C6_PC','Li_C7_Mel','Li_C8_TALSC',
                     'Li_C9_LSC','Li_C10_UN')
...
saveRDS(seur_obj, file = paste(resultsdir,"LiRNAannotated.rds"))
